    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="新建隧道" name="first">
        <el-form ref="form" :model="form" :rules="rules" label-width="150px" size="medium">
          <el-form-item label="隧道名称" prop="tunnelname">
            <el-input v-model="form.tunnelname" style="width: 50%;" />
          </el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" :loading="testFlag" @click="onSubmit">立即测试</el-button>
      ...
      <el-tab-pane label="隧道列表" name="second">
        <el-table stripe fit:true strip :data="tunnels.slice((currentPage - 1) * pageSize, currentPage * pageSize)">
          <el-table-column width="40px" prop="id" label="ID" />
          <el-table-column prop="tunnel_name" label="隧道名称" />
          <el-table-column prop="server_ip" label="目的IP" />
          <el-table-column prop="server_port" label="目的端口" />
          <el-table-column prop="status" label="隧道状态" />
          ...
          <el-table-column prop="cert_id" label="服务器证书" />
        ...
        <el-dialog title="隧道通信" :visible.sync="centerDialogVisible" width="30%" center>
          <el-form :model="contentform">
            <el-form-item label="消息内容" :label-width="formLabelWidth">
              <el-input v-model="contentform.info" autocomplete="off" />
            </el-form-item>
          </el-form>
